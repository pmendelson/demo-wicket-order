...
public class ViewCatalogPanel extends Panel {
  ...
  private class ProductProvider extends SortableDataProvider<Product> {
    ...
    public Iterator<? extends Product> iterator(int first, int count) {
      ...
      return pager
          ...
          .iterator(); // .getPage(first / pager.getPageSize()).getList().iterator();
    }
    ...
    private Query<Product> getQuery() {
      if (query == null) query = DB.getDefault().find(Product.class);
      return query;
    }
  ...
  public ViewCatalogPanel(String componentId) {
    ...
    add(
        new DefaultDataTable(ID4PRODUCT_LIST, getColumns(), dataProvider, 3)
  ...
  protected void createActionColumns(List<IColumn<Product>> r) {
    r.add(
        new AbstractColumn<Product>(new Model("")) {

          public void populateItem(
              Item<ICellPopulator<Product>> cellItem,
              String componentId,
            ...
            AjaxLink link =
                new AjaxLink("link") {
                  ...
                  protected void onComponentTagBody(
                    ...
                    String key =
                        getCart().hasItem(rowModel.getObject()) ? "prompt.drop" : "prompt.add";
